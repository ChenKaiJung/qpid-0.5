...
void ExchangeDeclareBody::setExchange(const string& _exchange) {
    exchange = _exchange;
    flags |= (1 << 8);
}
const string& ExchangeDeclareBody::getExchange() const { return exchange; }
bool ExchangeDeclareBody::hasExchange() const { return flags & (1 << 8); }
void ExchangeDeclareBody::clearExchangeFlag() { flags &= ~(1 << 8); }

void ExchangeDeclareBody::setType(const string& _type) {
    type = _type;
    flags |= (1 << 9);
}
const string& ExchangeDeclareBody::getType() const { return type; }
bool ExchangeDeclareBody::hasType() const { return flags & (1 << 9); }
void ExchangeDeclareBody::clearTypeFlag() { flags &= ~(1 << 9); }

void ExchangeDeclareBody::setAlternateExchange(const string& _alternateExchange) {
    alternateExchange = _alternateExchange;
    flags |= (1 << 10);
}
const string& ExchangeDeclareBody::getAlternateExchange() const { return alternateExchange; }
