#ifndef QPID_FRAMING_MESSAGEACQUIREBODY_H
#define QPID_FRAMING_MESSAGEACQUIREBODY_H
/*
 *
...
class MessageAcquireBody : public ModelMethod {
...
public:
    ...
    MessageAcquireBody(ProtocolVersion=ProtocolVersion())  : flags(0) {}
    
    void setTransfers(const SequenceSet& _transfers);
    const SequenceSet& getTransfers() const;
    bool hasTransfers() const;
    void clearTransfersFlag();
    typedef MessageAcquireResult ResultType;

    template <class T> ResultType invoke(T& invocable) const {
        return invocable.acquire(getTransfers());
    }
