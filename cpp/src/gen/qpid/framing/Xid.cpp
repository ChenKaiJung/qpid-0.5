...
void Xid::setGlobalId(const string& _globalId) {
    globalId = _globalId;
    flags |= (1 << 9);
}
const string& Xid::getGlobalId() const { return globalId; }
bool Xid::hasGlobalId() const { return flags & (1 << 9); }
void Xid::clearGlobalIdFlag() { flags &= ~(1 << 9); }

void Xid::setBranchId(const string& _branchId) {
    branchId = _branchId;
    flags |= (1 << 10);
}
const string& Xid::getBranchId() const { return branchId; }
