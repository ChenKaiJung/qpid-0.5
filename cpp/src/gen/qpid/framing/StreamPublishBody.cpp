...
#include "StreamPublishBody.h"

#include "reply_exceptions.h"

using namespace qpid::framing;

void StreamPublishBody::setExchange(const string& _exchange) {
    exchange = _exchange;
    flags |= (1 << 8);
}
const string& StreamPublishBody::getExchange() const { return exchange; }
bool StreamPublishBody::hasExchange() const { return flags & (1 << 8); }
void StreamPublishBody::clearExchangeFlag() { flags &= ~(1 << 8); }

void StreamPublishBody::setRoutingKey(const string& _routingKey) {
    routingKey = _routingKey;
    flags |= (1 << 9);
}
const string& StreamPublishBody::getRoutingKey() const { return routingKey; }
