...
void ReplyTo::setExchange(const string& _exchange) {
    exchange = _exchange;
    flags |= (1 << 8);
}
const string& ReplyTo::getExchange() const { return exchange; }
bool ReplyTo::hasExchange() const { return flags & (1 << 8); }
void ReplyTo::clearExchangeFlag() { flags &= ~(1 << 8); }

void ReplyTo::setRoutingKey(const string& _routingKey) {
    routingKey = _routingKey;
    flags |= (1 << 9);
}
const string& ReplyTo::getRoutingKey() const { return routingKey; }
