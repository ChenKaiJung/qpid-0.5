...
#include "StreamDeliverBody.h"

#include "reply_exceptions.h"

using namespace qpid::framing;

void StreamDeliverBody::setConsumerTag(const string& _consumerTag) {
    consumerTag = _consumerTag;
    flags |= (1 << 8);
}
const string& StreamDeliverBody::getConsumerTag() const { return consumerTag; }
...
void StreamDeliverBody::setExchange(const string& _exchange) {
    exchange = _exchange;
    flags |= (1 << 10);
}
const string& StreamDeliverBody::getExchange() const { return exchange; }
bool StreamDeliverBody::hasExchange() const { return flags & (1 << 10); }
void StreamDeliverBody::clearExchangeFlag() { flags &= ~(1 << 10); }

void StreamDeliverBody::setQueue(const string& _queue) {
    queue = _queue;
    flags |= (1 << 11);
}
const string& StreamDeliverBody::getQueue() const { return queue; }
