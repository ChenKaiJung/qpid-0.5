...
void ConnectionStartOkBody::setMechanism(const string& _mechanism) {
    mechanism = _mechanism;
    flags |= (1 << 9);
}
const string& ConnectionStartOkBody::getMechanism() const { return mechanism; }
bool ConnectionStartOkBody::hasMechanism() const { return flags & (1 << 9); }
void ConnectionStartOkBody::clearMechanismFlag() { flags &= ~(1 << 9); }

void ConnectionStartOkBody::setResponse(const string& _response) {
    response = _response;
    flags |= (1 << 10);
}
const string& ConnectionStartOkBody::getResponse() const { return response; }
bool ConnectionStartOkBody::hasResponse() const { return flags & (1 << 10); }
void ConnectionStartOkBody::clearResponseFlag() { flags &= ~(1 << 10); }

void ConnectionStartOkBody::setLocale(const string& _locale) {
    locale = _locale;
    flags |= (1 << 11);
}
const string& ConnectionStartOkBody::getLocale() const { return locale; }
