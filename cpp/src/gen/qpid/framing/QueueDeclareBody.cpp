...
#include "QueueDeclareBody.h"

#include "reply_exceptions.h"

using namespace qpid::framing;

void QueueDeclareBody::setQueue(const string& _queue) {
    queue = _queue;
    flags |= (1 << 8);
}
const string& QueueDeclareBody::getQueue() const { return queue; }
bool QueueDeclareBody::hasQueue() const { return flags & (1 << 8); }
void QueueDeclareBody::clearQueueFlag() { flags &= ~(1 << 8); }

void QueueDeclareBody::setAlternateExchange(const string& _alternateExchange) {
    alternateExchange = _alternateExchange;
    flags |= (1 << 9);
}
const string& QueueDeclareBody::getAlternateExchange() const { return alternateExchange; }
