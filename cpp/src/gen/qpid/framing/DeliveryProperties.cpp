...
void DeliveryProperties::setExchange(const string& _exchange) {
    exchange = _exchange;
    flags |= (1 << 0);
}
const string& DeliveryProperties::getExchange() const { return exchange; }
bool DeliveryProperties::hasExchange() const { return flags & (1 << 0); }
void DeliveryProperties::clearExchangeFlag() { flags &= ~(1 << 0); }

void DeliveryProperties::setRoutingKey(const string& _routingKey) {
    routingKey = _routingKey;
    flags |= (1 << 1);
}
const string& DeliveryProperties::getRoutingKey() const { return routingKey; }
bool DeliveryProperties::hasRoutingKey() const { return flags & (1 << 1); }
void DeliveryProperties::clearRoutingKeyFlag() { flags &= ~(1 << 1); }

void DeliveryProperties::setResumeId(const string& _resumeId) {
    resumeId = _resumeId;
    flags |= (1 << 2);
}
const string& DeliveryProperties::getResumeId() const { return resumeId; }
