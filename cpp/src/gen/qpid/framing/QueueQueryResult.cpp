...
#include "QueueQueryResult.h"

#include "reply_exceptions.h"

using namespace qpid::framing;

void QueueQueryResult::setQueue(const string& _queue) {
    queue = _queue;
    flags |= (1 << 8);
}
const string& QueueQueryResult::getQueue() const { return queue; }
bool QueueQueryResult::hasQueue() const { return flags & (1 << 8); }
void QueueQueryResult::clearQueueFlag() { flags &= ~(1 << 8); }

void QueueQueryResult::setAlternateExchange(const string& _alternateExchange) {
    alternateExchange = _alternateExchange;
    flags |= (1 << 9);
}
const string& QueueQueryResult::getAlternateExchange() const { return alternateExchange; }
