...
void StreamReturnBody::setReplyText(const string& _replyText) {
    replyText = _replyText;
    flags |= (1 << 9);
}
const string& StreamReturnBody::getReplyText() const { return replyText; }
bool StreamReturnBody::hasReplyText() const { return flags & (1 << 9); }
void StreamReturnBody::clearReplyTextFlag() { flags &= ~(1 << 9); }

void StreamReturnBody::setExchange(const string& _exchange) {
    exchange = _exchange;
    flags |= (1 << 10);
}
const string& StreamReturnBody::getExchange() const { return exchange; }
bool StreamReturnBody::hasExchange() const { return flags & (1 << 10); }
void StreamReturnBody::clearExchangeFlag() { flags &= ~(1 << 10); }

void StreamReturnBody::setRoutingKey(const string& _routingKey) {
    routingKey = _routingKey;
    flags |= (1 << 11);
}
const string& StreamReturnBody::getRoutingKey() const { return routingKey; }
