...
void AMQP_ClientProxy::Connection::secure(const string& challenge)
{
...
void AMQP_ClientProxy::Connection::redirect(const string& host, const Array& knownHosts)
{
...
void AMQP_ClientProxy::Connection::close(uint16_t replyCode, const string& replyText)
{
...
void AMQP_ClientProxy::Session::attach(const string& name, bool force)
{
    send(SessionAttachBody(getVersion(), name, force));
}
void AMQP_ClientProxy::Session::attached(const string& name)
{
    send(SessionAttachedBody(getVersion(), name));
}
void AMQP_ClientProxy::Session::detach(const string& name)
{
    send(SessionDetachBody(getVersion(), name));
}
void AMQP_ClientProxy::Session::detached(const string& name, uint8_t code)
{
...
void AMQP_ClientProxy::Execution::result(const SequenceNumber& commandId, const string& value)
{
    send(ExecutionResultBody(getVersion(), commandId, value));
}
void AMQP_ClientProxy::Execution::exception(uint16_t errorCode, const SequenceNumber& commandId, uint8_t classCode, uint8_t commandCode, uint8_t fieldIndex, const string& description, const FieldTable& errorInfo)
{
    send(ExecutionExceptionBody(getVersion(), errorCode, commandId, classCode, commandCode, fieldIndex, description, errorInfo));
}
void AMQP_ClientProxy::Message::transfer(const string& destination, uint8_t acceptMode, uint8_t acquireMode)
{
...
void AMQP_ClientProxy::Message::reject(const SequenceSet& transfers, uint16_t code, const string& text)
{
...
void AMQP_ClientProxy::Message::resume(const string& destination, const string& resumeId)
{
    send(MessageResumeBody(getVersion(), destination, resumeId));
}
void AMQP_ClientProxy::Message::setFlowMode(const string& destination, uint8_t flowMode)
{
    send(MessageSetFlowModeBody(getVersion(), destination, flowMode));
}
void AMQP_ClientProxy::Message::flow(const string& destination, uint8_t unit, uint32_t value)
{
    send(MessageFlowBody(getVersion(), destination, unit, value));
}
void AMQP_ClientProxy::Message::stop(const string& destination)
{
...
void AMQP_ClientProxy::File::consumeOk(const string& consumerTag)
{
    send(FileConsumeOkBody(getVersion(), consumerTag));
}
void AMQP_ClientProxy::File::open(const string& identifier, uint64_t contentSize)
{
...
void AMQP_ClientProxy::File::return_(uint16_t replyCode, const string& replyText, const string& exchange, const string& routingKey)
{
    send(FileReturnBody(getVersion(), replyCode, replyText, exchange, routingKey));
}
void AMQP_ClientProxy::File::deliver(const string& consumerTag, uint64_t deliveryTag, bool redelivered, const string& exchange, const string& routingKey, const string& identifier)
{
...
void AMQP_ClientProxy::Stream::consumeOk(const string& consumerTag)
{
    send(StreamConsumeOkBody(getVersion(), consumerTag));
}
void AMQP_ClientProxy::Stream::return_(uint16_t replyCode, const string& replyText, const string& exchange, const string& routingKey)
{
    send(StreamReturnBody(getVersion(), replyCode, replyText, exchange, routingKey));
}
void AMQP_ClientProxy::Stream::deliver(const string& consumerTag, uint64_t deliveryTag, const string& exchange, const string& queue)
{
