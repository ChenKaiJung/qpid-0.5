...
#include "ExchangeBindBody.h"

#include "reply_exceptions.h"

using namespace qpid::framing;

void ExchangeBindBody::setQueue(const string& _queue) {
    queue = _queue;
    flags |= (1 << 8);
}
const string& ExchangeBindBody::getQueue() const { return queue; }
bool ExchangeBindBody::hasQueue() const { return flags & (1 << 8); }
void ExchangeBindBody::clearQueueFlag() { flags &= ~(1 << 8); }

void ExchangeBindBody::setExchange(const string& _exchange) {
    exchange = _exchange;
    flags |= (1 << 9);
}
const string& ExchangeBindBody::getExchange() const { return exchange; }
bool ExchangeBindBody::hasExchange() const { return flags & (1 << 9); }
void ExchangeBindBody::clearExchangeFlag() { flags &= ~(1 << 9); }

void ExchangeBindBody::setBindingKey(const string& _bindingKey) {
    bindingKey = _bindingKey;
    flags |= (1 << 10);
}
const string& ExchangeBindBody::getBindingKey() const { return bindingKey; }
