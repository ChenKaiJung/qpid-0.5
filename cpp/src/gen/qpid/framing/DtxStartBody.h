#ifndef QPID_FRAMING_DTXSTARTBODY_H
#define QPID_FRAMING_DTXSTARTBODY_H
/*
 *
...
class DtxStartBody : public ModelMethod {
    Xid xid;
    uint16_t flags;
public:
    ...
    DtxStartBody(ProtocolVersion=ProtocolVersion())  : flags(0) {}
    
    void setXid(const Xid& _xid);
    const Xid& getXid() const;
    bool hasXid() const;
    void clearXidFlag();
    void setJoin(bool _join);
    bool getJoin() const;
    ...
    template <class T> ResultType invoke(T& invocable) const {
        return invocable.start(getXid(), getJoin(), getResume());
    }
...
#endif  /*!QPID_FRAMING_DTXSTARTBODY_H*/
