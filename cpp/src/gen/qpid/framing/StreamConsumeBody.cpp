...
#include "StreamConsumeBody.h"

#include "reply_exceptions.h"

using namespace qpid::framing;

void StreamConsumeBody::setQueue(const string& _queue) {
    queue = _queue;
    flags |= (1 << 8);
}
const string& StreamConsumeBody::getQueue() const { return queue; }
bool StreamConsumeBody::hasQueue() const { return flags & (1 << 8); }
void StreamConsumeBody::clearQueueFlag() { flags &= ~(1 << 8); }

void StreamConsumeBody::setConsumerTag(const string& _consumerTag) {
    consumerTag = _consumerTag;
    flags |= (1 << 9);
}
const string& StreamConsumeBody::getConsumerTag() const { return consumerTag; }
