...
#include "FilePublishBody.h"

#include "reply_exceptions.h"

using namespace qpid::framing;

void FilePublishBody::setExchange(const string& _exchange) {
    exchange = _exchange;
    flags |= (1 << 8);
}
const string& FilePublishBody::getExchange() const { return exchange; }
bool FilePublishBody::hasExchange() const { return flags & (1 << 8); }
void FilePublishBody::clearExchangeFlag() { flags &= ~(1 << 8); }

void FilePublishBody::setRoutingKey(const string& _routingKey) {
    routingKey = _routingKey;
    flags |= (1 << 9);
}
const string& FilePublishBody::getRoutingKey() const { return routingKey; }
...
void FilePublishBody::setIdentifier(const string& _identifier) {
    identifier = _identifier;
    flags |= (1 << 12);
}
const string& FilePublishBody::getIdentifier() const { return identifier; }
