...
#include "FileDeliverBody.h"

#include "reply_exceptions.h"

using namespace qpid::framing;

void FileDeliverBody::setConsumerTag(const string& _consumerTag) {
    consumerTag = _consumerTag;
    flags |= (1 << 8);
}
const string& FileDeliverBody::getConsumerTag() const { return consumerTag; }
...
void FileDeliverBody::setExchange(const string& _exchange) {
    exchange = _exchange;
    flags |= (1 << 11);
}
const string& FileDeliverBody::getExchange() const { return exchange; }
bool FileDeliverBody::hasExchange() const { return flags & (1 << 11); }
void FileDeliverBody::clearExchangeFlag() { flags &= ~(1 << 11); }

void FileDeliverBody::setRoutingKey(const string& _routingKey) {
    routingKey = _routingKey;
    flags |= (1 << 12);
}
const string& FileDeliverBody::getRoutingKey() const { return routingKey; }
bool FileDeliverBody::hasRoutingKey() const { return flags & (1 << 12); }
void FileDeliverBody::clearRoutingKeyFlag() { flags &= ~(1 << 12); }

void FileDeliverBody::setIdentifier(const string& _identifier) {
    identifier = _identifier;
    flags |= (1 << 13);
}
const string& FileDeliverBody::getIdentifier() const { return identifier; }
