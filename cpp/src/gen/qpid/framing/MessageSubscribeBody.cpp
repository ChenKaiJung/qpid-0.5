...
#include "MessageSubscribeBody.h"

#include "reply_exceptions.h"

using namespace qpid::framing;

void MessageSubscribeBody::setQueue(const string& _queue) {
    queue = _queue;
    flags |= (1 << 8);
}
const string& MessageSubscribeBody::getQueue() const { return queue; }
bool MessageSubscribeBody::hasQueue() const { return flags & (1 << 8); }
void MessageSubscribeBody::clearQueueFlag() { flags &= ~(1 << 8); }

void MessageSubscribeBody::setDestination(const string& _destination) {
    destination = _destination;
    flags |= (1 << 9);
}
const string& MessageSubscribeBody::getDestination() const { return destination; }
...
void MessageSubscribeBody::setResumeId(const string& _resumeId) {
    resumeId = _resumeId;
    flags |= (1 << 13);
}
const string& MessageSubscribeBody::getResumeId() const { return resumeId; }
