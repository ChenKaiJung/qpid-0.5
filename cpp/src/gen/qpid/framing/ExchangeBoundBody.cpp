...
#include "ExchangeBoundBody.h"

#include "reply_exceptions.h"

using namespace qpid::framing;

void ExchangeBoundBody::setExchange(const string& _exchange) {
    exchange = _exchange;
    flags |= (1 << 8);
}
const string& ExchangeBoundBody::getExchange() const { return exchange; }
bool ExchangeBoundBody::hasExchange() const { return flags & (1 << 8); }
void ExchangeBoundBody::clearExchangeFlag() { flags &= ~(1 << 8); }

void ExchangeBoundBody::setQueue(const string& _queue) {
    queue = _queue;
    flags |= (1 << 9);
}
const string& ExchangeBoundBody::getQueue() const { return queue; }
bool ExchangeBoundBody::hasQueue() const { return flags & (1 << 9); }
void ExchangeBoundBody::clearQueueFlag() { flags &= ~(1 << 9); }

void ExchangeBoundBody::setBindingKey(const string& _bindingKey) {
    bindingKey = _bindingKey;
    flags |= (1 << 10);
}
const string& ExchangeBoundBody::getBindingKey() const { return bindingKey; }
