...
void AMQP_ServerProxy::Connection::startOk(const FieldTable& clientProperties, const string& mechanism, const string& response, const string& locale)
{
    send(ConnectionStartOkBody(getVersion(), clientProperties, mechanism, response, locale));
}
void AMQP_ServerProxy::Connection::secureOk(const string& response)
{
...
void AMQP_ServerProxy::Connection::open(const string& virtualHost, const Array& capabilities, bool insist)
{
...
void AMQP_ServerProxy::Connection::close(uint16_t replyCode, const string& replyText)
{
...
void AMQP_ServerProxy::Session::attach(const string& name, bool force)
{
    send(SessionAttachBody(getVersion(), name, force));
}
void AMQP_ServerProxy::Session::attached(const string& name)
{
    send(SessionAttachedBody(getVersion(), name));
}
void AMQP_ServerProxy::Session::detach(const string& name)
{
    send(SessionDetachBody(getVersion(), name));
}
void AMQP_ServerProxy::Session::detached(const string& name, uint8_t code)
{
...
void AMQP_ServerProxy::Execution::result(const SequenceNumber& commandId, const string& value)
{
    send(ExecutionResultBody(getVersion(), commandId, value));
}
void AMQP_ServerProxy::Execution::exception(uint16_t errorCode, const SequenceNumber& commandId, uint8_t classCode, uint8_t commandCode, uint8_t fieldIndex, const string& description, const FieldTable& errorInfo)
{
    send(ExecutionExceptionBody(getVersion(), errorCode, commandId, classCode, commandCode, fieldIndex, description, errorInfo));
}
void AMQP_ServerProxy::Message::transfer(const string& destination, uint8_t acceptMode, uint8_t acquireMode)
{
...
void AMQP_ServerProxy::Message::reject(const SequenceSet& transfers, uint16_t code, const string& text)
{
...
void AMQP_ServerProxy::Message::resume(const string& destination, const string& resumeId)
{
    send(MessageResumeBody(getVersion(), destination, resumeId));
}
void AMQP_ServerProxy::Message::subscribe(const string& queue, const string& destination, uint8_t acceptMode, uint8_t acquireMode, bool exclusive, const string& resumeId, uint64_t resumeTtl, const FieldTable& arguments)
{
    send(MessageSubscribeBody(getVersion(), queue, destination, acceptMode, acquireMode, exclusive, resumeId, resumeTtl, arguments));
}
void AMQP_ServerProxy::Message::cancel(const string& destination)
{
    send(MessageCancelBody(getVersion(), destination));
}
void AMQP_ServerProxy::Message::setFlowMode(const string& destination, uint8_t flowMode)
{
    send(MessageSetFlowModeBody(getVersion(), destination, flowMode));
}
void AMQP_ServerProxy::Message::flow(const string& destination, uint8_t unit, uint32_t value)
{
    send(MessageFlowBody(getVersion(), destination, unit, value));
}
void AMQP_ServerProxy::Message::flush(const string& destination)
{
    send(MessageFlushBody(getVersion(), destination));
}
void AMQP_ServerProxy::Message::stop(const string& destination)
{
...
void AMQP_ServerProxy::Exchange::declare(const string& exchange, const string& type, const string& alternateExchange, bool passive, bool durable, bool autoDelete, const FieldTable& arguments)
{
    send(ExchangeDeclareBody(getVersion(), exchange, type, alternateExchange, passive, durable, autoDelete, arguments));
}
void AMQP_ServerProxy::Exchange::delete_(const string& exchange, bool ifUnused)
{
    send(ExchangeDeleteBody(getVersion(), exchange, ifUnused));
}
void AMQP_ServerProxy::Exchange::query(const string& name)
{
    send(ExchangeQueryBody(getVersion(), name));
}
void AMQP_ServerProxy::Exchange::bind(const string& queue, const string& exchange, const string& bindingKey, const FieldTable& arguments)
{
    send(ExchangeBindBody(getVersion(), queue, exchange, bindingKey, arguments));
}
void AMQP_ServerProxy::Exchange::unbind(const string& queue, const string& exchange, const string& bindingKey)
{
    send(ExchangeUnbindBody(getVersion(), queue, exchange, bindingKey));
}
void AMQP_ServerProxy::Exchange::bound(const string& exchange, const string& queue, const string& bindingKey, const FieldTable& arguments)
{
    send(ExchangeBoundBody(getVersion(), exchange, queue, bindingKey, arguments));
}
void AMQP_ServerProxy::Queue::declare(const string& queue, const string& alternateExchange, bool passive, bool durable, bool exclusive, bool autoDelete, const FieldTable& arguments)
{
    send(QueueDeclareBody(getVersion(), queue, alternateExchange, passive, durable, exclusive, autoDelete, arguments));
}
void AMQP_ServerProxy::Queue::delete_(const string& queue, bool ifUnused, bool ifEmpty)
{
    send(QueueDeleteBody(getVersion(), queue, ifUnused, ifEmpty));
}
void AMQP_ServerProxy::Queue::purge(const string& queue)
{
    send(QueuePurgeBody(getVersion(), queue));
}
void AMQP_ServerProxy::Queue::query(const string& queue)
{
...
void AMQP_ServerProxy::File::consume(const string& queue, const string& consumerTag, bool noLocal, bool noAck, bool exclusive, bool nowait, const FieldTable& arguments)
{
    send(FileConsumeBody(getVersion(), queue, consumerTag, noLocal, noAck, exclusive, nowait, arguments));
}
void AMQP_ServerProxy::File::cancel(const string& consumerTag)
{
    send(FileCancelBody(getVersion(), consumerTag));
}
void AMQP_ServerProxy::File::open(const string& identifier, uint64_t contentSize)
{
...
void AMQP_ServerProxy::File::publish(const string& exchange, const string& routingKey, bool mandatory, bool immediate, const string& identifier)
{
...
void AMQP_ServerProxy::Stream::consume(const string& queue, const string& consumerTag, bool noLocal, bool exclusive, bool nowait, const FieldTable& arguments)
{
    send(StreamConsumeBody(getVersion(), queue, consumerTag, noLocal, exclusive, nowait, arguments));
}
void AMQP_ServerProxy::Stream::cancel(const string& consumerTag)
{
    send(StreamCancelBody(getVersion(), consumerTag));
}
void AMQP_ServerProxy::Stream::publish(const string& exchange, const string& routingKey, bool mandatory, bool immediate)
{
