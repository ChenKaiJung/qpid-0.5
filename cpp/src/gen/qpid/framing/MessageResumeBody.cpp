...
#include "MessageResumeBody.h"

#include "reply_exceptions.h"

using namespace qpid::framing;

void MessageResumeBody::setDestination(const string& _destination) {
    destination = _destination;
    flags |= (1 << 8);
}
const string& MessageResumeBody::getDestination() const { return destination; }
bool MessageResumeBody::hasDestination() const { return flags & (1 << 8); }
void MessageResumeBody::clearDestinationFlag() { flags &= ~(1 << 8); }

void MessageResumeBody::setResumeId(const string& _resumeId) {
    resumeId = _resumeId;
    flags |= (1 << 9);
}
const string& MessageResumeBody::getResumeId() const { return resumeId; }
