...
void FileConsumeBody::setQueue(const string& _queue) {
    queue = _queue;
    flags |= (1 << 8);
}
const string& FileConsumeBody::getQueue() const { return queue; }
bool FileConsumeBody::hasQueue() const { return flags & (1 << 8); }
void FileConsumeBody::clearQueueFlag() { flags &= ~(1 << 8); }

void FileConsumeBody::setConsumerTag(const string& _consumerTag) {
    consumerTag = _consumerTag;
    flags |= (1 << 9);
}
const string& FileConsumeBody::getConsumerTag() const { return consumerTag; }
