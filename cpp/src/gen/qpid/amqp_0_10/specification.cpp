...
ConfigChange::ConfigChange(const Vbin16& current_) :
    current(current_){
}
...
const char* MessageExpired::NAME="cluster.message-expired";
const char* MessageExpired::CLASS_NAME=cluster::NAME;

MessageExpired::MessageExpired(Uint64 id_) :
    id(id_){
}
void MessageExpired::accept(Visitor& v) {  v.visit(*this); }
void MessageExpired::accept(ConstVisitor& v) const { v.visit(*this); }

std::ostream& operator << (std::ostream& o, const MessageExpired&x) {
    o << "cluster.message-expired[";
    o << " id=" << x.id;
    o << "]";
    return o;
}
void MessageExpired::Handler::clusterMessageExpired(
    Uint64 /*id_*/
)
{
    assert(0);
...
std::ostream& operator << (std::ostream& o, const Shutdown&) {
    o << "cluster.shutdown[";
    o << "]";
    return o;
}
...
const char* Queue::NAME="cluster-connection.queue";
const char* Queue::CLASS_NAME=cluster_connection::NAME;

Queue::Queue(const Str32& encoded_) :
    encoded(encoded_){
}
void Queue::accept(Visitor& v) {  v.visit(*this); }
void Queue::accept(ConstVisitor& v) const { v.visit(*this); }

std::ostream& operator << (std::ostream& o, const Queue&x) {
    o << "cluster-connection.queue[";
    o << " encoded=" << x.encoded;
    o << "]";
    return o;
}
void Queue::Handler::clusterConnectionQueue(
    const Str32& /*encoded_*/
)
{
    assert(0);
    throw NotImplementedException(QPID_MSG("cluster-connection.queue not implemented."));
}

const char* ExpiryId::NAME="cluster-connection.expiry-id";
const char* ExpiryId::CLASS_NAME=cluster_connection::NAME;

ExpiryId::ExpiryId(Uint64 expiryId_) :
    expiryId(expiryId_){
}
void ExpiryId::accept(Visitor& v) {  v.visit(*this); }
void ExpiryId::accept(ConstVisitor& v) const { v.visit(*this); }

std::ostream& operator << (std::ostream& o, const ExpiryId&x) {
    o << "cluster-connection.expiry-id[";
    o << " expiry-id=" << x.expiryId;
    o << "]";
    return o;
}
void ExpiryId::Handler::clusterConnectionExpiryId(
    Uint64 /*expiryId_*/
)
{
    assert(0);
