...
class QueuePolicy
{
  ...
  public:
    ...
    virtual ~QueuePolicy() {}
    void tryEnqueue(boost::intrusive_ptr<Message> msg);
    void recoverEnqueued(boost::intrusive_ptr<Message> msg);
    ...
    static std::auto_ptr<QueuePolicy> createQueuePolicy(const std::string& name, const qpid::framing::FieldTable& settings);
    static std::auto_ptr<QueuePolicy> createQueuePolicy(const std::string& name, uint32_t maxCount, uint64_t maxSize, const std::string& type = REJECT);
