...
int main(int argc, char** argv) {
    const char* host = argc>1 ? argv[1] : "61.67.137.215";
    int port = argc>2 ? atoi(argv[2]) : 5672;
    const char* id = argc>3 ? argv[3] : "anonymous";
    const char* pwd = argc>4 ? argv[4] : "aa1234";
    int count = argc>5 ? atoi(argv[5]) : 100000;
  	
    Connection connection;
    try {
        connection.open(host, port, id, pwd);
